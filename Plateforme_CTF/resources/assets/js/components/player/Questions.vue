        ...
        <div v-for="(q, index) in questions" class="row form-inline">
          <div class="col-md-12 question-item">
            ...
            <div v-else class="input-group pull-right">
              ...
              <div v-else>
                <input type="text" class="form-control" v-model="user_answer[index]" :key="index" placeholder="Ecrire le flag ici " />
                <div class="input-group-btn">
                  <a v-on:click="submitAnswer($event,q,index)" class="btn btn-primary">Soumettre</a>
                </div>
                <div class="input-group-addon">
                  <span v-if="wrong_answer[q.question_id]" class="fa fa-times text-red"> Faux</span> {{ q.question_points }} Points
                </div>
              </div>
            </div>
